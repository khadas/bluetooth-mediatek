...
int boots_uart_init(char *dev, int speed)
{
    ...
    if (0 == speed) {
        ...
        if (strcmp(usb_prop, prop_value)) {
            if (property_set(VCOM_PROP_NAME, prop_value) < 0) {
#endif
                if (access(CHECK_VCOM_DEV, F_OK) == 0) {
                    if (boots_system(OPEN_VCOM_FOR_TTYGS2) < 0) {
                ...
                if (access(CHECK_VCOM_DEV_FOR_NEW_USB_1, F_OK) == 0 ||
                    ...
                    if (boots_system(OPEN_VCOM_FOR_TTYGS2_NEW_USB) < 0) {
                        BPRINT_E("Open default VCOM failed!");
                        return -1;
                    } else {
                        BPRINT_D("Open default VCOM successfully");
                    }
                }
#ifdef OPEN_VCOM_BY_SET_PROP
            } else {
