...
static pkt_list_s *mtk_boots_cmd_ble_tx(char *cmd[], size_t len)
{
    ...
    while (i + 1 < len) {
        ...
        case 'p':   // Pattern
            ...
            break;
        case 'l':  // length
            ble_tx[5] = strtol(cmd[i + 1], NULL, 16);
            i += 2;
            break;
        default:
            BPRINT_E("%s: Incorrect format: %s", __func__, cmd[i]);
...
static pkt_list_s *mtk_boots_cmd_tx_power_offset(char *cmd[], size_t len)
{
    ...
    while (i + 1 < len) {
        ...
        case 'g': {     // Tone Type
            ...
            txpoweroffset[gpidx + 4] = strtol(cmd[i + 1], NULL, 10);
            i += 2;
            } break;
...
static pkt_list_s *mtk_boots_cmd_set_buffer_mode(char *cmd[], size_t len)
{
    ...
    while (i + 1 < len) {
        ...
        case 'f':
            ...
            memset(path_cp, 0x00, sizeof(path_cp));
...
BLUE"      Can replace this by ble_etx, EX: \"ble_etx -y 1 ...\"\n"NONE \
    "      [c] Channel: 0-39\n" \
    "      [l] Length in bytes of packet: 0-255(Default is 37 if ignore this)\n" \
    "      [p] Pattern:\n" \
...
#define USAGE_BLEETX_DETAILS \
    "      [c] Channel: 0-39\n" \
    "      [l] Length in bytes of packet: 0-255\n" \
    "      [p] Payload of packet:\n" \
    "          0:PRBS9      1:11110000      2:10101010  3:PRBS15\n" \
    "          4:11111111   5:00000000      6:00001111  7:01010101\n" \
    "      [y] Phy:\n" \
...
boots_cmds_s commands[] = {
    ...
    { "pa", mtk_boots_cmd_set_pa, "Set ePA or iPA <0:iPA/1:ePA>", USAGE_PA_DETAILS, false },
